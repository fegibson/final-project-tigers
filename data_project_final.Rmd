...
tiger_attack_why <- conflict_df_clean %>% 
  separate_longer_delim(perception_why_tigers_attack_humans, delim = " ") %>% 
  mutate(perception_why_tigers_attack_humans = as.factor(perception_why_tigers_attack_humans))

# also saving as ANOTHER object because i don't have time for ERRORS!!!!! we need to see the levels in order to fix some rows that decided to get Funky(tm) while separating
```

```{r}
levels(tiger_attack_why$perception_why_tigers_attack_humans)
```
...
human_tiger_attacks %>%
  ggplot(aes(x = year)) +
  geom_line(aes(y = tigers_killed, color = "tigers_killed"), size = 1.5) +
  geom_line(aes(y = humans_attacked, color = "humans_attacked"), size = 1.5) +
